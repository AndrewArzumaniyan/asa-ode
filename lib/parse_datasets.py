...
def parse_datasets(args, device):
	...
	if dataset_name == "lorenz96":
		dataset_obj = Lorenz96(root='data', generate=True, noise_weight=args.noise_weight, device=device)
		dataset = dataset_obj.get_dataset()[:args.n]
		dataset = dataset.to(device)
		...
		if not args.extrap:
			...
			sliced = []
			for i in range(n_traj):
				sliced.append(dataset[i, start_ind[i]:start_ind[i] + n_reduced_tp, :])
			dataset = torch.stack(sliced).to(device)
			time_steps = time_steps[:n_reduced_tp]
	...
	##################################################################
	# Coupled Oscillators dataset
	if dataset_name == "oscillators":
		dataset_obj = CoupledOscillators(root='data', generate=True, noise_weight=args.noise_weight, device=device)
		dataset = dataset_obj.get_dataset()[:args.n]
		dataset = dataset.to(device)
	...
	if dataset_name == "spiral":
		dataset_obj = SpiralDataset(root='data', generate=True, noise_weight=args.noise_weight, device=device)
		dataset = dataset_obj.get_dataset()[:args.n]
		dataset = dataset.to(device)
