...
parser.add_argument('--dataset', type=str, default='periodic', help="Dataset to load. Available: physionet, activity, hopper, periodic, lorenz96, oscillators")
...
if __name__ == '__main__':
	...
	epoch_bar = tqdm(range(1, args.niters + 1), desc="Training", unit="epoch")
	for epoch in epoch_bar:
		batch_bar = tqdm(range(num_batches), desc=f"Epoch {epoch:04d}", leave=False, unit="batch")
		for _ in batch_bar:
			itr = (epoch - 1) * num_batches + _ + 1
			optimizer.zero_grad()
			...
			batch_bar.set_postfix(loss=f"{train_res['loss'].item():.4f}")

		epoch_bar.set_postfix(loss=f"{train_res['loss'].item():.4f}")

		n_iters_to_viz = 1
		if epoch % n_iters_to_viz == 0:
			with torch.no_grad():
